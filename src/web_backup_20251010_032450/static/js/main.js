...
function switchEnvironment(env) {
    ...
    const targetBtn = document.querySelector(`[data-env="${env}"]`);
    if (targetBtn) {
    ...
    // Add visual feedback
    if (window.WARPCORE && window.WARPCORE.addTerminalLine) {
        window.WARPCORE.addTerminalLine('System', `🔄 Environment switched to: ${env.toUpperCase()}`, 'success');
    }
}
...
function showLicenseModal() {
    ...
    let modal = document.querySelector('.license-modal');
    if (!modal) {
        ...
        modal.innerHTML = `
            <div class="modal-content" style="background: white; padding: 20px; border-radius: 8px; max-width: 500px;">
                ...
                <p>License management interface is now available.</p>
                <button onclick="closeLicenseModal()" style="padding: 8px 16px; margin-top: 10px;">Close</button>
            </div>
        `;
...
function refreshLicenseStatusDirect() {
    console.log('Refreshing license status');
    
    // Add visual feedback
    if (window.WARPCORE && window.WARPCORE.addTerminalLine) {
        window.WARPCORE.addTerminalLine('System', '🔄 Refreshing license status...', 'info');
    }
...
function executeCommand(command) {
    ...
    // Add visual feedback
    if (window.WARPCORE && window.WARPCORE.addTerminalLine) {
        window.WARPCORE.addTerminalLine('Command', `$ ${command}`, 'info');
    }
...
window.refreshLicenseStatusDirect = refreshLicenseStatusDirect;
